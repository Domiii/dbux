...
export function waitTicksCallback(t, next) {
  repeatCallback(t, next);
}

export function repeatCallback(condition, next = noop) {
  condition = normalizeCondition(condition);
  if (condition()) {
    setImmediate(function nestedRepeat() { repeatCallback(condition, next) });
  }
  else {
    next();
  }

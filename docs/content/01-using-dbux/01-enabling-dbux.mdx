...
2. The [@dbux/cli](../tools-and-configuration/dbux-cli) provides the magic that makes [The Run Button](./the-run-button) <DarkLightImg src='play.svg' /> work. You can use it directly in order to run simple non-bundled Node applications.
...
4. In order to enable Dbux for frontend and other bundled applications the developer needs to TODO [build pipeline integration](../tools-and-configuration/build-pipeline-integration).

Once running, an instrumented target application will try to record and send all runtime data to the [runtime server](../tools-and-configuration/dbux-code#runtime-server) from where [runtime analysis](#runtime-analysis) can commence.

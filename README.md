...
5. [Known Issues & Limitations](#known-issues--limitations)
   ...
   3. [`eval`'ed code will not be traced](#evaled-code-will-not-be-traced)
...
6. [Architectural Notes](#architectural-notes)
   1. [The Call Graph HTML GUI](#the-call-graph-html-gui)
...
If you have any questions, feel free to [join us on DISCORD](https://discord.gg/bdD3yH).

# Using Dbux

The easiest way to start with Dbux is through the [Dbux VSCode Plugin](dbux-code).

NOTE: [Dbux VSCode Plugin](dbux-code) is also (currently) the only frontend for Dbux. So even if you follow the steps below, you are likely going to need it, unless you want to perform [Programmatic Data Analysis](#programmatic-data-analysis) on the resulting data sets.

While you definitely want to get started with the VSCode plugin, you might encounter scenarios where the "Run" buttons are not enough anymore. In that case you might have to manually inject the [@dbux/babel-plugin](dbux-babel-plugin), which is explained in the next section.


# Adding Dbux to your build pipeline

In order to analyze you runtime, your program must be instrumented with Dbux and injected with the [@dbux/runtime](dbux-runtime). [@dbux/babel-plugin](dbux-babel-plugin) is responsible for both, meaning you need to "[babel](https://babeljs.io/) your program" with [@dbux/babel-plugin](dbux-babel-plugin) enabled.

There are two primary ways:

1. Use the [@dbux/cli](dbux-cli) (command line interface) which in turn uses [@babel/register](https://babeljs.io/docs/en/babel-register) to instrument code on the fly. [Read more here](dbux-cli).
1. Add the [@dbux/babel-plugin](dbux-babel-plugin) (IMPORTANT: as the **last** entry in your `plugins` array) to your build pipeline manually. [Read more here](dbux-babel-plugin).
...
* Example: When just running `var public = 3;` in `node` or the browser, you don't get an error. However when running the same code with [@dbux/cli](dbux-cli) (which is also invoked when pressing the "Run" button), it gives throws a synxtax error.

NOTE: You can see the same problem when slightly modifying above example and running it in `node` (without Dbux) but with strict mode enabled: `"use strict"; var public = 3;`.
...
* That is because:
   ...
   1. By default, babel treats js files as [ESModules](https://nodejs.org/api/esm.html) (or `esm`s), and ESModules have strict mode enabled by default. This is also discussed here: https://github.com/babel/babel/issues/7910
* You should be able to customize the babel config and disable strict mode if you please. However we recommend to just work in strict mode to begin with.


## Issues under Windows

* An entirely unrelated bugs occurs **very rarely**, when running things in VSCode built-in terminal, it might change to lower-case drive letter.
...
1. [`dbux-code`](dbux-code) The Dbux VSCode extension ([VSCode marketplace link](https://marketplace.visualstudio.com/items?itemName=Domi.dbux-code)). You can also install it from within VSCode via the "Extensions" panel.
1. [`@dbux/projects`](dbux-practice) Used by `@dbux/code` (while not dependending on `VSCode`) to allow practicing dbux (and debugging in general) on real-world bugs inside of real-world open source projects.
...
* It consists of three modules [`@dbux/graph-common`](dbux-graph-common), [`@dbux/graph-client`](dbux-graph-client) and [`@dbux/graph-host`](dbux-graph-host).

...
export default class FirestoreContainer {
  ...
  constructor(db) {
    ...
    setTimeout(() => {
      ...
      if (!this.state._state) {
        throw new Error(this.constructor.n + ': this.state has been overwritten. Make sure not to re-declare `state` as a class variable in a FirebaseContainer class.');
      }
  ...
  defaultQueryMap = snap => {
    if (snap instanceof this.db.firebase.firestore.QuerySnapshot) {
      // array of docs
  ...
  _queryRead = (registration, ...args) => {
    ...
    if (argsPath.length > 40) {
      // eslint-disable-next-line no-console
      console.warn(`[POSSIBLE PERFORMANCE ISSUE] ${fullName} has a big argument set, argsPath length > 40: ${argsPath}`);
    }

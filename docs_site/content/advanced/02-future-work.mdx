...
NOTE: The code should still work fine, but some of Dbux's analysis tools (especially [Data Flow](../runtime-analysis/11-data-flow.mdx)) will not be able to provide all actual connections.


## Problems with Values

Because of [performance](../guides/03-performance.mdx) reasons, we cannot record *everything*.
...
`@dbux/runtime` is currently hard-coded to connect to a `localhost` server (see <CodeLink path="dbux-runtime/src/client/Client.js" />).
...
3. By default, `plugin-transform-modules-commonjs` treats `*.js` files as [ESModules](https://nodejs.org/api/esm.html) (or `esm`s), and ESModules have `strict mode` enabled by default. This is also discussed here: https://github.com/babel/babel/issues/7910 -->

## Future Work: Missing Features

This list serves to keep track of features that could prove very valuable for runtime analysis and debugging purposes. 

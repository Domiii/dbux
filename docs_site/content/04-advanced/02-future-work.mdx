...
Example: When just running `var public = 3;` in `node` or the browser, you don't get an error. However when running the same code with [@dbux/cli](../tools-and-configuration/dbux-cli) (which is also invoked when pressing the `Dbux VSCode Extension`'s "Run with Dbux" button), it throws a synxtax error.
...
* Typescript support
* Remote capabilities
  * NOTE: `@dbux/runtime` is currently hardcoded to connect to a `localhost` server (see <CodeLink path="dbux-runtime\src\client\Client.js" />).

...
* Visualize [recursion trees](https://www.google.com/search?q=recursion+trees), like in the screenshot of the [fibbonacci number](https://www.google.com/search?q=fibonacci+algorithm) generator below:
...
The **call stack** is the list of all executed functions that have not yet concluded, and are not currently suspended (e.g. by `await` or `yield`), at a specific point in time. While useful, the call stack only represents a small fraction of our application. In fact, the call stack is a small slice of the call graph during its [depth-first traversal](https://en.wikipedia.org/wiki/Depth-first_search), at a specific point in time.
...
## Asynchronouse Call Graph {async}

...
import HintVolta from './_install-hint-volta.mdx';
import HintNode from './_install-hint-node.mdx';
...
The [Dbux VSCode Extension](../tools-and-configuration/01-dbux-code.mdx) requires several system tools to be installed to function. It automatically tries its best to determine whether the tools it requires are installed.

At least it requires `Node@16` and `npm`. If it is missing anything, it should warn you when starting. If nothing is missing, you should see a message like this in the Dbux Output Channel:
...
:::caution
When facing an issue with your Node version, [please read on below](#node-version).
:::
...
When working through [Dbux Practice](../dbux-practice/01-overview.mdx) exercises, more tools are required:
...
:::caution
This is a [PSA](https://en.wikipedia.org/wiki/Public_service_announcement):

[Don't Install Node Packages Globally](https://www.google.com/search?q=dont+install+node+packages+globally&hl=en)!
...
If you have already done so, that is of course also Ok. It just needs a lot of careful attention when you want to use different Node versions at any point in time.
:::

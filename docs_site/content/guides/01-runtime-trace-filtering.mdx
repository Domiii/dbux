...
When [Dbux is enabled](../runtime-analysis/02-enable-dbux.mdx), it automatically records execution of all files that are not in `node_modules`[^1]. This can be configured. However, it is a balancing act to determine the optimal set of files to trace. Often times it is easiest to tell Dbux to just trace everything, but tracing *everything* can slow things down significantly.

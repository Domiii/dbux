...
:::tip
If you know that some packages do not contribute to your runtime analysis, do not `whitelist` them or explicitly `blacklist` them. This can improve [performance](./02-performance.mdx), especially if they contain many lines of code or many <Term term="trace" /> event records.
:::
...
You can disable tracing of individual traces and/or expressions by adding a comment containing a line of `dbux disable`, (e.g. `/** dbux disable */`) in front of it. E.g., in the following sample code, a long-running (but unimportant) loop will not get instrumented, thus largely improving [performance](./02-performance.mdx):

```js
f();
/* dbux disable */
for (const i = 0; i < veryLargeNumber; ++i) {
  // ...
}
g();
```

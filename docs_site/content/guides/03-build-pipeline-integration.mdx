...
const dbuxOptions = {
  moduleFilter: {
    packageWhitelist: '.*',
    // NOTE: these libraries hook themselves into webpack and run before dbux is initialized, so they must not be instrumented
    packageBlacklist: 'react-dev-utils,react-refresh,react-error-overlay,process',
    fileWhitelist: '.*',

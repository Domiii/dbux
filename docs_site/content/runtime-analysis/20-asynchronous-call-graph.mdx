...
The following screenshot demonstrates an ACG of a sample program:

<Img screen src="dbux-all-async1.png" />

The ACG (right) renders the program [CGRs](#cgr) and the [asynchronous event edges](#ae) that connect them. The vertical axis is time (pointing downwards) and the horizontal dimension is "virtual threads" or "degree of concurrency" (experimental measure).

* Code: The red solid border around the `done()` call expression indicates that it is [selected](./05-select-trace.mdx).
* In the ACG, the `await Promise.all(...)` is also automatically selected (due to [follow mode](./08-call-graph.mdx#follow)). 
  * &rarr; This means that `done()` is in the CGR of that corresponding asynchronous continuation (or "virtual/resume context"). 
  * &rarr; This also means that `done()` executed synchronously after the awaited promise settled.
* The dashed (red) border around the nodes on the left (`f1`, `sleep1s`, `f2`) indicate that the selected node "[synchronizes](https://en.wikipedia.org/wiki/Synchronization_(computer_science))" against them.
  * &rarr; Those nodes were scheduled earlier but also had to finish before this root could start executing.
* Other observations:
  * `f2` will always follow `f1`, `g2` will always follow `g1` and `h2` will always `h1`.
  * `f`, `g` and `h` are executing concurrently, and have no mechanism to ensure that one will finish before the other.
...
In order to better understand how the ACG works, we start by introducing relevant terminology as it pertains to JavaScript's asynchronous semantics.

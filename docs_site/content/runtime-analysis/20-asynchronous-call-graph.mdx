...
The Asynchronous Call Graph (ACG) is the [call graph](./08-call-graph.mdx) but in `Async` mode. You can see the ACG by opening the [call graph](./08-call-graph.mdx) and then clicking the `Sync` button. The two call graph modes share many of the toolbar features, as explained [here](./08-call-graph.mdx#toolbar).

Here, you can see many high-level patterns, that the synchronous call graph cannot capture. While `Sync` mode allows inspecting children and children of children of file and function executions, `Async` mode focuses on relating the roots of the graph to one another.
...
The ACG (right) renders the program [CGRs](#cgr) and the [asynchronous event edges](#ae) that connect them. The vertical axis is time (pointing downwards) and the horizontal dimension is a new experimental measure of "virtual threads" or "degree of concurrency".
...
* Other observations:
  * `f2` will always follow `f1`, `g2` will always follow `g1` and `h2` will always `h1`.
  * `f`, `g` and `h` are executing concurrently. Presented with what we can see from the screenshot, we cannot assume that there is any ordering between them.
  * &rarr; If someone wrote code that works based on the assumption that `h` always finishes after `g`, or `f` after `h`, there might be a [race condition](https://en.wikipedia.org/wiki/Race_condition) because there is no such order assured in the code that we are presented with. That is why the three are rendered on separate "virtual threads" (vertical columns).
...
* We have enabled [value mode](./08-call-graph.mdx#value).
  * This renders the first execution of the currently selected trace's value in each <Term term="CGR" />.
* NOTE: Event handler invocations of the same event are rendered in the same column.
  * TODO: explain *callback heuristics* in more detailed.
...
* Continuation of an async function, after an `await` expression.
* Execution of a JavaScript file, e.g. when `require`d or `import`ed by `node` or the webpack (or other) bundling system.

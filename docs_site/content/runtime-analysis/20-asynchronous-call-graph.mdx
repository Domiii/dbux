...
`Sync` mode allows inspecting children and children of children of file and function executions. However, it simply puts <Term term="call graph roots" /> on a linear (vertical) timeline. The ACG, on the other hand, uncovers the hidden connections between those roots.
...
With the ACG we hope to make asynchronous JavaScript a lot easier to understand and digest. It presents a clear picture of an application's asynchronous control flow, and makes many types of race conditions visually obvious. For example: if your application's ACG has many individual columns, but a very low level of concurrency, it might indicate that you forgot to `await` or otherwise chain or nest your promises.
<!-- TODO: better/more examples for visible race conditions -->

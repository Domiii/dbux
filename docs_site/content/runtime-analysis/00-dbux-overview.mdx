---
slug: /runtime-analysis
---

# Dbux Overview

import Img from '@src/components/Img';
import DarkLightImg from '@src/components/DarkLightImg';
import CodeLink from '@src/components/CodeLink';
import ToolLink from '@src/components/ToolLink';
import TOC from '@src/components/TOC';

<TOC toc={toc} />

This page introduces Dbux at the high level, while the next few pages explain how to use the <ToolLink name="dbux-code" /> for runtime analysis.


## Dbux Architecture

As of 01/2022, Dbux consists of four applications and seven supplementary modules:

<Img zoomable src="dbux-architecture.png" />

<!-- from https://docs.google.com/presentation/d/15uSkBoZsBjoPOLtx23iEACaEwR4buyKF5qNqygwvhsg/edit#slide=id.gaea430f77a_0_0 -->

Dbux has three stages `instrument`, `runtime` and `post-processing`, implemented in four collaborating applications:



* &#8203;<ToolLink name="dbux-babel-plugin" /> instruments and injects <ToolLink name="dbux-runtime" /> a given application.
* &#8203;<ToolLink name="dbux-runtime" /> When an instrumented program runs, this module is responsible for recording and sending runtime data to the `@dbux/data` module, running on a receiving server (using `socket-io.client`).
* &#8203;<ToolLink name="dbux-code">dbux-code</ToolLink> is a one-click-installable VSCode Extension. It hosts the run-time server to receive the data produced by `dbux-runtime`. When data is received it is post-processed with the help of the `dbux-data` module before allowing the user to investigate it. Data is processed and presented in real-time, meaning that applications can be debugged while they are still running.
* &#8203;<ToolLink name="dbux-cli" />, a convenience CLI tool, allows executing a Node application with Dbux enabled using [@babel/register](https://babeljs.io/docs/en/babel-register).


### Utilities and Libraries

Several utilities and libraries are also part of the [Dbux Monorepo](https://github.com/Domiii/dbux):

* &#8203;<CodeLink path="dbux-graph-common" />, <CodeLink path="dbux-graph-client" /> and <CodeLink path="dbux-graph-host" /> are responsible for rendering and letting the user interact with the "Call Graph" through an HTML GUI.
* &#8203;<CodeLink path="dbux-data" /> Receives, pre-processes and manages all data sent by `@dbux/runtime`. It allows us to query and analyze JS runtime data on a higher level.
* &#8203;<CodeLink path="dbux-projects" /> is used by `dbux-code` as part of [Dbux Practice](../dbux-practice) (but does not depend on `VSCode`). It allows users to clone, install and execute real-world projects, their bugs and practice exercises *with a single click*.
* &#8203;<CodeLink path="dbux-common" /> Collection of commonly used utilities shared among (more or less) all other modules.
* &#8203;<CodeLink path="dbux-common-node" /> Collection of commonly used utilities shared among (more or less) all node-only modules.


## Runtime Analysis

Once [installed](./runtime-analysis/installation), using Dbux involves two steps:

1. First: make sure to [run an Application with Dbux enabled](./02-enable-dbux.mdx).
2. Then use the Dbux VSCode Extension for [runtime analysis](./00-dbux-overview.mdx#runtime-analysis):

All recorded applications show up in the [Application View](./runtime-analysis/06-applications.mdx).
Once the application shows up, you can use the following tools for analysis:

* Use the [Application View](./runtime-analysis/applications) to overview all recorded applications.
  * From here you can also go to the ***entry point***.
* Get a high-level overview with:
  * The [Global View](./runtime-analysis/global).
  * The [Call Graph](./runtime-analysis/call-graph).
* Get down into the nitty-gritty:
  * &#8203;<DarkLightImg src="crosshair_red.svg" /> [Select a Trace](./05-select-trace.mdx).
  * Investigate individual traces using the [Trace Details View](./runtime-analysis/trace-details).
  * &#8203;<DarkLightImg width="24px" src="nextInContext.svg" /> <DarkLightImg width="24px" src="previousInContext.svg" /> [Navigate](./runtime-analysis/trace-details#navigation) between traces.
  * [Search](./runtime-analysis/search) for traces, contexts, modules or data.
* Understand where data came from and where it went using the [Data Flow View](./runtime-analysis/data-flow).

:::tip
All buttons in the Dbux VSCode Extension have corresponding [commands](./tools-and-configuration/dbux-code#commands) which in turn (are not by default but) [can be key-bound](https://code.visualstudio.com/docs/getstarted/keybindings).
:::

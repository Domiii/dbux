...
* **Roots**: By default, the synchronous call graph shows a list of all <Term term="CGR">call graph roots</Term>: the entry point of the application, as well as the starting point of any <Term term="ae" />, vertically sorted by time of recording (later is lower).

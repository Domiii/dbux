...
In `Async` mode, the call graph becomes the asynchronous call graph, which is explained on [the next page](./20-asynchronous-call-graph.mdx).
...
The **call stack** is the list of all executed functions that have not yet concluded and are not currently suspended<sup><span className="cursor-help" title="e.g. by await or yield">?</span></sup>, at a specific point in time. While useful, the call stack only represents a small fraction of our application. In fact, the call stack can be defined as a slice of the call graph during its [depth-first traversal](https://en.wikipedia.org/wiki/Depth-first_search), at a specific point in time.

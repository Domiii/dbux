...
0. I wanted to be able to answer (seemingly always the same type of) questions, like: How did THAT happen? Where did THAT data come from? Where did the execution take THAT turn?
1. I wanted to actually **see** (not just [guess/assume/theorize/conjecture/suppose/opine/test/prod/verify](https://www.thesaurus.com/browse/guess)) what is going on.
2. I wanted to interact more with the runtime structure, not just indirectly through print statements, or one single small step at a time. I wanted to be able to explore some of the non-obvious connections from where I am to where the bug is.
3. I wanted to see the whole thing, zoom out all the way to gain a big-picture overview.
4. I wanted to be able to zoom in real close and see all details regarding that one piece of code.

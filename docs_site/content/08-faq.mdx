...
In order to analyze an application, it first needs to be [recorded](./using-dbux/02-enable-dbux.mdx) and sub-sequently show up in [the application view](./using-dbux/06-applications.mdx).

There can be a multitude of possible reasons for this to go wrong:

1. You did not instrument the application. Make sure to use one of the four methods of [Running a JavaScript Application w/ Dbux Enabled](./using-dbux/02-enable-dbux.mdx).

...
Call Graph Roots (CGRs) are the ***nodes*** of Dbux's [Asynchronous Call Graph (ACG)](../runtime-analysis/08-call-graph.mdx#async). By comparison, in the `synchronous` [call graph](../runtime-analysis/08-call-graph.mdx) lists all CGRs on a linear timeline.
...
Asynchronous Events (AEs) are the ***edges*** in Dbux's [Asynchronous Call Graph (ACG)](../runtime-analysis/08-call-graph.mdx#async). They connect pieces of executed code that can never appear together in the same (non-asynchronous) call stack. Any AE `e` has at least three properties:

...
In the following example, `f` has two AEs `E1` and `E2`. `E2` is always a CHAIN, but `E1` might be CHAIN or FORK, depending on the caller. Assume that the example codes ex1-ex4 are all at root-level:
...
<div className="grid-2-col mb-05">
  <CodeBlock className="border-purple" lang="js">{`
// ex1: E1 is FORK
A; f(); B;
  `}</CodeBlock>
  <CodeBlock className="border-purple" lang="js">{`
...
Below are several more illustrations of asynchronous programs and their expected conceptual ACG with CHAINs (blue) and FORKs (purple):
...
In JavaScript, promises can be created in five ways. Somewhat counter-intuitively, (1), (2) and (3) do *not* cause an asynchronous event on their own.
...
5. [Dynamic imports](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/import\#Dynamic_Imports) and other native functions create and return promises.
   * NOTE: The <Term term="ACG" /> does not yet handle dynamic imports or top-level `awaits` correctly.

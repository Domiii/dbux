...
:::tip Important
[The Runtime Analysis manual](../runtime-analysis/) explains the GUI and all analysis features in detail.
:::

This page goes over technical expension-specific topics:


## Runtime Server

Currently, the VSCode Extension is the only implementation of the <DarkLightImg src="antenna_green.svg" /> Dbux Runtime Server <DarkLightImg src="antenna_red.svg" />. If enabled, it receives data from [Dbux-enabled applications](../runtime-analysis/02-enable-dbux.mdx) running on `localhost`[^1]. Once a first batch of data is received, the application will show up in the [Application View](../runtime-analysis/applications).

The Runtime Server is turned off by default. It turns on automatically the first time you use [the Run Button](../runtime-analysis/03-the-run-button.mdx) or run a [Dbux Practice exercise](../dbux-practice/01-overview.mdx).

You can manually turn it on or off via "the Runtime Server button" at the top of [the Applications view](../runtime-analysis/06-applications.mdx)[^2]:

...
:::tip
This page goes into some lower level details while [the Runtime Analysis manual](../runtime-analysis/) explains the GUI and all analysis features in detail.
:::


## Runtime Server

Currently, the VSCode Extension is the only implementation of the <DarkLightImg src="antenna_green.svg" /><DarkLightImg src="antenna_red.svg" /> Dbux Runtime Server. If enabled, it receives data from [Dbux-enabled applications](../runtime-analysis/02-enable-dbux.mdx) running on `localhost`[^1]. Once a first batch of data is received, the application will show up in the [Application View](../runtime-analysis/applications).

...
The Dbux CLI package `@dbux/cli` is a tool for easily running Node applications [with Dbux enabled](../runtime-analysis/02-enable-dbux.mdx).
...
* The `--pw` flag is one type of ***module filter***. It whitelists 3rd party packages (from `node_modules`) to also be instrumented and traced.
  * More explanations on module filters can be found [in the trace guide](../guides/01-which-files-are-traced.mdx).
...
`node --stack-trace-limit=100    --require "./test/support/env.js" "./node_modules/@dbux/cli/bin/dbux.js" run --verbose=1 --cache --sourceRoot=C:/Users/domin/code/dbux --pw=.* "C:/Users/domin/code/dbux/dbux_projects/express/node_modules/mocha/bin/_mocha" -- --no-exit -c -t 10000 --grep "OPTIONS should only include each method once" --grep "OPTIONS should only include each method once" -- test/app.options.js test/app.options.js`

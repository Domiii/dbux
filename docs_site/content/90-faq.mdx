...
In order to analyze an application, it first needs to be [recorded](./dbux-features/02-enable-dbux.mdx) and sub-sequently show up in [the application view](./dbux-features/05-applications.mdx).

There can be a multitude of possible reasons for this to go wrong:

1. You did not instrument the application. Make sure to use one of the four methods of [Running a JavaScript Application w/ Dbux Enabled](./dbux-features/02-enable-dbux.mdx).
...
If you are certain that a file executed in a [Dbux-enabled application](./dbux-features/02-enable-dbux.mdx), make sure your [configuration is set up to include](./guides/01-runtime-trace-filtering.mdx) the file in that specific package or path.
